...
const maskClasses: Record<BGMaskType, string> = {
	'fade-edges': '[mask-image:radial-gradient(ellipse_at_center,var(--background)_15%,transparent_60%)]',
...
function geBgImage(variant: BGVariantType, fill: string, size: number) {
	switch (variant) {
		...
		case 'grid':
			return `linear-gradient(to right, ${fill} 0.5px, transparent 0.5px), linear-gradient(to bottom, ${fill} 0.5px, transparent 0.5px)`;
